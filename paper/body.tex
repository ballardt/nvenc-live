...
Talk about dynamic\_bitstream and the fact that NVENC is predictable.
...
\begin{table*}[t]
	...
	\begin{tabular}{llp{3in}r}
		...
		& \texttt{loop\_filter\_across\_tiles\_enabled\_flag} & Whether in-loop filtering should be performed across tile boundaries. This is desired in our case because each tile represents one part of a single logical image. & \texttt{1} \\
		\midrule[1pt]
		...
		& \texttt{slice\_segment\_address} & The address of the top-leftmost CTU in this tile. CTU addresses begin at 0 for the top-leftmost CTU in the whole image, and increment from left to right along the image, moving down and beginning from the left again once the right-hand edge of the image is reached. Note that CTU addresses ignore tile structure and are instead based on the whole image. & \\ \cmidrule[0.5pt]{2-4}
